  ...
  private
    ...
    def handle_subscribe_event
      if @keyword.present?
        ...
        return reply_text_message("扫描带参数二维码事件: 1. 用户(#{@weixin_message.to_s})未关注时，进行关注后的事件推送, keyword: #{@keyword}")
      end
    ...
    def handle_scan_event
      ...
      # end    
      reply_text_message("#{@weixin_public_account.to_s}扫描带参数二维码事件: 2. 用户(#{@weixin_message.to_s})已关注时的事件推送, keyword: #{@keyword}")
    end
